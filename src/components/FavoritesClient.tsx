...
import { Heart, Trash2, Package, Wrench, Target, BookOpen, Home, ClipboardList } from "lucide-react";
import { FavoriteButton } from "@/components/FavoriteButton";

export function FavoritesClient({ data }: { data: ContentData }) {
  const { weapons, attachments, loot: lootItems, missions, guides } = data;
      ...
      <div className="mx-auto max-w-7xl px-4 py-20">
        <div className="text-center">
          ...
          <p className="mt-3 text-zinc-400">
            Добавляйте оружие, лут, квесты, гайды и бункеры в избранное, чтобы быстро находить их здесь
          </p>
          <Link
            href="/"
  ...
  const missionFavorites = favorites
    .filter((f) => f.type === "mission")
    .map((f) => missions.find((m) => m.slug === f.slug))
    .filter(Boolean);
      ...
      {missionFavorites.length > 0 && (
        <section className="mb-10">
          <h2 className="mb-4 flex items-center gap-2 text-2xl font-black text-white">
            ...
            Квесты ({missionFavorites.length})
          </h2>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {missionFavorites.map((mission) => (
              <Link key={mission.slug} href={`/missions/${mission.slug}`} className="group relative rounded-2xl border border-zinc-800 bg-zinc-950/80 p-5 transition-all hover:border-red-500/50 hover:bg-zinc-900">
                <div className="absolute right-4 top-4"><FavoriteButton type="mission" slug={mission.slug} /></div>
                <div className="mb-3 flex flex-wrap gap-2 pr-10"><span className="rounded-full bg-red-500/20 px-3 py-1 text-xs font-bold text-red-500">Tier {mission.tier}</span><span className="rounded-full bg-zinc-800 px-3 py-1 text-xs font-bold text-zinc-300">{mission.trader}</span></div>
                <h3 className="text-xl font-black text-white">{mission.title}</h3>
                <p className="mt-2 text-sm text-zinc-400">{mission.short}</p>
              </Link>
            ))}
          </div>
        </section>
