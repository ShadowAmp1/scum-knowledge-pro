...
## 6. После перезапуска хостинга

После рестарта сервера проверь:

1. Главная страница открывается.
2. /admin открывается и принимает актуальный логин/пароль из переменных окружения.
3. /admin/status показывает PostgreSQL подключенным.
4. /map, /weapons, /loot, /guides, /missions показывают данные из базы.
